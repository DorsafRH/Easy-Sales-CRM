package com.crm.modules.reporting.security;
...
/**
 * Garde d'authentification machine-à-machine pour les endpoints de reporting
 * automatisé (appelés par n8n, sans JWT).
 *
 * <p>Vérifie l'en-tête {@code X-Callback-Secret} contre le secret dédié
 * {@code reporting.callback-secret} (externalisé). La comparaison est à temps
 * constant pour ne pas exposer la longueur/préfixe du secret via le timing.</p>
 *
 * <p>Choix Phase A : secret partagé simple (auth machine-à-machine, moindre
 * privilège, périmètre isolé). Durcissement futur possible : HMAC-SHA256
 * (intégrité + secret jamais transmis).</p>
 *
 * @author Riahi Dorsaf
 */
@Slf4j
@Component
public class CallbackSecretGuard {

    public static final String HEADER = "X-Callback-Secret";

    private final byte[] secretAttendu;

    public CallbackSecretGuard(@Value("${reporting.callback-secret}") String secret) {
    ...
    public void verifier(String secretFourni) {
        if (!StringUtils.hasText(secretFourni)
            ...
            log.warn("Appel reporting automatisé rejeté : secret partagé absent ou invalide.");
